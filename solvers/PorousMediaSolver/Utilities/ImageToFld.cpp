...
int main(int argc, char* argv[])
{
    ...
    if(argc != 4)
    {
        fprintf(stderr,"Usage: ImageToFld session.xml singleimage000 (no tif extension) out.fld \n");
        exit(1);
    }
    ...
    try
    {
        ...
        vtkImageReader->SetDataExtent(0,159,0,599,0,299);
        ...
        for(int i = 0; i < nq; ++i)
        {
            ...
            //coordinates of the quadrature points (scaled) (1.6, 6, 6), (1.6,6,5)
            p[0]=(xc0[i]/1.6)*(dim[0]-1)*whl[0]; 
            p[1]=(xc1[i]/6)*(dim[1]-1)*whl[1];
            p[2]=(xc2[i]/3)*(dim[2]-1);
