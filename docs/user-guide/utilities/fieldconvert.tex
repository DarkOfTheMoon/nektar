...
\item \inltt{addfieldfromstring}: Add a new field to file using an input string definition defined by required ``fieldstr'' argument;
...
\subsection{Add new fields from a string: \textit{addfieldfromstring} module}
This module will add a new field variable using a given string
definition. The new field can be a combination of the coordinates
``x,y,z'' and the variables of the fld file that is provided. For
example if file1.fld contains the variables ``{\em u,v,p}'' you can add a
new field defined as ``{\em x + y + u}'' by specifying the required argument
\inltt{fieldstr} using the command:

\begin{lstlisting}[style=BashInputStyle]
  FieldConvert -m addfieldfromstring:fieldstr=''x+y+u'' file1.xml file1.fld \
               file2.fld
\end{lstlisting}


There is also an optional argument \inltt{fieldname} which allows you to give the new field a specified name, i..e

\begin{lstlisting}[style=BashInputStyle]
  FieldConvert -m addfieldfromstring:fieldstr=''x+y+u'':fieldname=''myvar'' \
               file1.xml file1.fld file2.fld
\end{lstlisting}
