...
The directory containing \nekpp will be refered as \bold{\$NEK} for the remainder of the User Guide. (e.g. if the \nekpp is installed in the home directory with name "nektar++", \$NEK will refer to $\sim$/nektar++)
...
\subsection{Third Party Software Installation}

Third party software can be installed manually by downloading the installation files from the official websites. Here are the list of websites you may visit:

\begin{itemize}
\item CMake \texttt{https://cmake.org/}
\item BLAS \texttt{http://www.netlib.org/blas/}
\item LAPACK \texttt{http://www.netlib.org/lapack/}
\item Boost \texttt{http://www.boost.org/}
\item ModMETIS \texttt{https://www.nektar.info/thirdparty/}
\item FFTW \texttt{http://www.fftw.org/}
\item ARPACK \texttt{http://www.caam.rice.edu/software/ARPACK/}
\item OpenMPI \texttt{https://www.open-mpi.org/}
\item GSMPI \texttt{http://ae-nektar.ae.ic.ac.uk/ dmoxey/test/gsmpi-1.1/}
\item PETSc \texttt{https://www.mcs.anl.gov/petsc/}
\item Scotch \texttt{http://gforge.inria.fr/projects/scotch/}
\item VTK \texttt{http://www.vtk.org/}
\end{itemize}

You may also find all of the above at \texttt{https://www.nektar.info/thirdparty/} \textbf{(supposed to add description to the softwares and move the CMake component selection here as of what is discussed)}
...
Details of the programs and libraries for the functionality of \nekpp can be found in \ref{s:installation:source:cmake}.
...
tar -zxvf nektar++-(*\nekver*).tar.gz
    \end{lstlisting}
    \item Change into the source-code directory
    \begin{lstlisting}[style=BashInputStyle]
    cd $NEK
\end{lstlisting}
    create a \inltt{build}
    ...
    \item Each time a change is made in the options of CMake (e.g. switching on/off an option), the software has to be installed again following the above procedures.
\end{enumerate}
...
Details of the options in CMake can be found in \ref{s:installation:source:cmake}.
...
Package names are given in the table above and can be installed by replacing cmake with other names (e.g. arpack/openmpi). Similar packages also exist in other
...
\begin{itemize}
\item What if the OpenMPI is not working? \\ Enter the command in terminal
    \begin{lstlisting}[style=BashInputStyle]
sudo port select mpi openmpi-mp-fortran 
sudo port select mpi
    \end{lstlisting}
    Test if OpenMPI works by entering the command
        \begin{lstlisting}[style=BashInputStyle]
mpirun
    \end{lstlisting}
\end{itemize}
...
\subsection{CMake Option Reference (to be moved to the front)}
...
\begin{itemize}
    ...
    \item \inlsh{NEKTAR\_USE\_ARPACK} (Recommended)
    ...
    implemented directly in \nekpp. It is recommended to install this package as most of the calculations will involve the use of ARPACK library.
    ...
    serial or parallel. Running the solvers in parallel allows many calculations to be carried out simultaneously, reducing the time needed for rendering the result.\\

    If OpenMPI does not work when it is fetched automatically from the source, it is possible to install it manually by downloading the software from the official website \url{https://www.open-mpi.org/software/ompi/v2.0/}
