...
\section{FieldConvertCFS}

There is a directory within the Compressible Flow Solver called
FieldConvert that contains a extended version of FieldConvert called
\inltt{FieldConvertCFS}. This is a full version of FieldConvert
discussed in section \ref{s:utilities:fieldconvert} which also
contains solver specific modules:

\begin{enumerate}
\item \inltt{nonconservativevariables}: Convert a field file to non-conservative variables. 
\end{enumerate}
\subsection{Convert field to non-conservative variables: \textit{nonconservativevariables} module }

This module will convert a field file which is output in conservative variables to non-conservative variables. If you have a file.xml and a file.fld file you run this module to create a new field file file2.fld,  by running the command:

\begin{lstlisting}[style=BashInputStyle]
  FieldConvertCFS -m nonconservativevariables file1.xml file1.fld file2.fld
\end{lstlisting}
